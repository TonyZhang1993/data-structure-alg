...
在HTTP/1.0中,需要在请求头中显式添加"Connection: keep-alive"来启用该机制; 
在HTTP/1.1中,keep-alive是默认启用的. 增加了持久连接，就是说之前版本的协议一次请求就是一次TCP连接，请求完成后这个连接就关闭掉了； 还引入了管道机制，就是说在一个TCP连接里可以同时发送多个HTTP请求
...
*多路复用: HTTP/2引入了多路复用技术,允许在单个TCP连接中并行处理多个请求和响应. 这消除了HTTP/1.x中的队头阻塞问题,极大地提高了网络性能和资源利用率. 【同一个连接并发处理多个请求，而且并发请求的数量比HTTP1.1大了好几个数量级。】

*头部压缩: HTTP/2使用了头部压缩技术,通过共享头部信息,可以显著减少传输的数据量. 这有助于减少延迟和网络带宽的消耗,特别是在传输大量小请求时效果更为显著. 【HTTP2.0使用HPACK算法对header的数据进行压缩，数据体积小了，在网络上传输就会更快】
...
---TCP/IP协议
应用层 HTTP,DNS,FTP,SMTP,NFS,telnet

传输层 TCP【segment段】,UDP【数据报】 【TCP对应的是可靠性要求高的应用，而UDP对应的则是可靠性要求低、传输经济的应用】
...
HTTP 特点
1 无状态，每个请求独立，但可以引入cookie 关联请求
2 基于TCP 协议， 默认持久连接，多次 http 请求 使用一个tcp 连接

状态码
200 ok
301 Moved Permanently 永久重定向
302 found 临时重定向
304 Not Modified 资源没有变动，通知浏览器使用缓存资源

404 Not Found 表明请求的资源在服务器上找不到
403 Forbidden 表明请求访问的资源被拒绝
...
存储位置：
Cookie：存储在客户端（浏览器）中，以文本文件形式存储在用户的计算机上，每次请求时会被发送到服务器。
Session：存储在服务器上，通常存储在内存中或者数据库中，不会直接暴露给客户端。

内容：
Cookie：通常包含用户身份验证、跟踪信息或个性化设置等数据，大小受限制，每个域名下的cookie数也有限制。
Session：通常包含用户的会话数据、状态信息等，可以存储较大量的数据，因为存储在服务器上。

生命周期：
Cookie：可以设置过期时间，可以是会话级别（关闭浏览器后失效）或持久性的（指定过期日期后失效）。
Session：会话通常在用户关闭浏览器后失效，但可以通过持久化存储（如数据库）来延长会话的有效期。

安全性：
Cookie：可以通过设置HttpOnly和Secure标志增强安全性，防止被JavaScript访问以及仅通过HTTPS传输。
Session：存储在服务器上，相对于Cookie更安全，不易受到客户端篡改或窃取。

跨域：
Cookie：可以设置域名，允许在同一域名下的不同路径之间共享。
Session：通常与特定的应用实例相关联，不同应用间的会话不共享，除非通过额外的机制实现。
